...
st.title("🚕 WroTaxi Compare v5.8")

# --- LOGIKA CZASOWA ---
now = datetime.now()
h = (now.hour + 1) % 24  # Korekta strefy czasowej jeśli potrzebna
...
elif (11.0 <= time_val <= 14.2):
    t_status = "🍴 RUCH PRZEDPOŁUDNIOWY / LUNCH"
    u_base, u_km = 8.00, 2.10
    b_base, b_km = 4.80, 2.70
    # Dynamiczny skok Ubera po 12:25 (Lunch Peak we Wrocławiu)
    u_surge = 1.16 if time_val >= 12.4 else 1.0
    surge = 1.0 # Bolt pozostaje stabilny
elif is_peak:
...
def get_data():
    try:
        return openrouteservice.Client(key=ORS_KEY), Nominatim(user_agent="wrotaxi_v58_precision")
        ...
        with st.spinner("Przeliczanie..."):
            try:
                ...
                if l1 and l2:
                    ...
                    # Uber reaguje na u_surge (skok po 12:30)
                    uber_x = ((u_base + (km * u_km) + (dur * 0.15)) * u_surge) * u_mult
                    # Bolt ma stałą opłatę serwisową i własny surge
                    ...
                    dane = [
                        {
                            ...
                            "Vars": [
                                ...
                                ("🚗 UberX", uber_x), 
                                ("🔋 Hybrid", uber_x * 1.01), 
                                ("✨ Comfort", uber_x * (1.44 if u_surge > 1.1 else 1.18))
                            ]
                        },
                        {
                            "Firma": "Bolt ⚡",
