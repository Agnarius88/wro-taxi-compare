...
# --- KLUCZ API (Wklej tutaj swój NOWY klucz!) ---
ORS_KEY = '5b3ce3597851100001cf6248383f98c603b544d6935272a912a2335f'

def init_services():
    try:
        client = openrouteservice.Client(key=ORS_KEY)
        geolocator = Nominatim(user_agent="wro_taxi_color_v1")
...
if st.button("SPRAWDŹ CENY"):
    if start_adr and cel_adr:
        with st.spinner("Szukam najtańszej opcji..."):
            try:
                ...
                if l1 and l2:
                    ...
                    dane = [
                        {"Firma": "UberX 🚗", "Cena": f"~{8.0 + km*2.5:.2f} PLN", "Link": f"https://m.uber.com/ul/?action=setPickup&pickup[latitude]={l1.latitude}&pickup[longitude]={l1.longitude}&pickup[nickname]={q_start}&dropoff[latitude]={l2.latitude}&dropoff[longitude]={l2.longitude}&dropoff[nickname]={q_cel}", "Val": 8.0 + km*2.5},
                        {"Firma": "iTaxi 🚕", "Cena": f"~{7.0 + km*3.0:.2f} PLN", "Link": f"https://itaxi.pl/zamow-przejazd/?address_from={q_start}&address_to={q_cel}", "Val": 7.0 + km*3.0},
                        {"Firma": "Bolt ⚡", "Cena": f"~{6.5 + km*2.8:.2f} PLN", "Link": "bolt://ride", "Val": 6.5 + km*2.8},
                    ...
                    st.error("Nie znalazłem adresu.")
            except Exception as e:
                st.error(f"Limit API lub błąd: {e}")
    else:
